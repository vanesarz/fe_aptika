...
export default function DetailView({ data, apiUrl, onBack }: DetailViewProps) {
  ...
  // State baru untuk tombol copy Alamat Repository
  const [isRepoCopied, setIsRepoCopied] = useState(false);

  // Fungsi untuk menyalin alamat repository
  const handleCopyRepo = async () => {
    if (!data.alamat_repository) return;
    try {
      await navigator.clipboard.writeText(data.alamat_repository);
      setIsRepoCopied(true);
      setTimeout(() => setIsRepoCopied(false), 2000); // Reset ikon copy setelah 2 detik
    } catch (err) {
          ...
          <div className="w-[800px] h-[1131px] shrink-0 bg-white shadow-md border border-slate-300 p-12 flex flex-col text-black">
            <div className="flex mb-4 border border-black">
              <div className="w-[15%] p-2 border-r border-black flex flex-col items-center justify-center text-center">
                <img src="/logo.png" alt="Logo" className="object-contain h-auto w-14" />
                <p className='text-[7px]'>DISKOMINFO<br></br>PROVINSI JAWA BARAT</p>
              </div>
            ...
            <div className="bg-[#F8FAFC] rounded-xl p-6 border border-slate-100">
              ...
              <div className="grid grid-cols-1 mb-6 md:grid-cols-2 gap-y-6 gap-x-8">
                ...
                <div>
                  ...
                  {data.alamat_aplikasi ? (
                    <a
                      href={data.alamat_aplikasi.startsWith('http') ? data.alamat_aplikasi : `https://${data.alamat_aplikasi}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-[13px] font-medium text-blue-600 hover:text-blue-800 hover:underline break-all inline-flex items-center gap-1"
                    >
                      {data.alamat_aplikasi}
                      <svg viewBox="0 0 24 24" className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M10 6H6a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
                    </a>
                  ) : (
 ...
 <div>
                ...
                <div className="flex items-center justify-between bg-white border border-slate-200 rounded-md overflow-hidden focus-within:border-[#113289] transition-colors">
                  <div className="font-mono text-[12px] text-slate-700 px-4 py-2.5 break-all flex-1 bg-slate-50/50">
                  ...
                  {data.alamat_repository && (
                    <button
                      ...
                      className="p-2.5 text-slate-400 hover:text-[#113289] hover:bg-slate-100 transition-colors shrink-0 border-l border-slate-200 flex items-center justify-center bg-white"
            ...
            <div className="mt-8">
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                {[
                  ...
                  { label: 'Keterangan Risiko Bila Tidak Dilakukan', value: data.keterangan },
